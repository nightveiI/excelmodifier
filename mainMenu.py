...
def convert(file, selectedSettings):
    ...
    if(isSubtype):
        ...
        for i in range(1, len(originalArray)):
            if(originalArray[i][dividingIndex] not in dividingCategories):
                dividingCategories.append(originalArray[i][dividingIndex])
            ...
            if(isSubtype):
                if(modifiedArray[i][dividingIndex] != None):
                    if(modifiedArray[i][dividingIndex].find('Shingle') != -1):
                        print("here in shingle")
                        newWB.sheets["Shingle"].range('A' + str(i+1)).value = modifiedArray[i]
                    elif(modifiedArray[i][dividingIndex].find('Metal') != -1):
                        print("here in metal")
                        newWB.sheets["Metal"].range('A' + str(i+1)).value = modifiedArray[i]
                    elif(modifiedArray[i][dividingIndex].find('TPO') != -1):
                        print("here in tpo")
                        newWB.sheets["TPO"].range('A' + str(i+1)).value = modifiedArray[i]
                    elif(modifiedArray[i][dividingIndex].find('Coating') != -1):
                        print("here in coating")
                        newWB.sheets["Coating"].range('A' + str(i+1)).value = modifiedArray[i]
            else:
                for x in range(len(dividingCategories)):
    ...
    newWB.save(r'converted_' + d + '.xlsx')
    newWB.close()
    dfWB.close()
